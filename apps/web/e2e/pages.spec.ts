...
test.describe('Pages', () => {
  ...
  test('scheduled jobs page toggle all', async ({ page }) => {
    ...
    const expectedLabel = initialLabel.includes('Collapse All') ? 'Expand All' : 'Collapse All'
  ...
  test('failed job retry streams status in modal and requeues job', async ({ page }) => {
    ...
    await expect
      .poll(
        async () => {
          const job = await getJob(page, connectionId, failedJob.queueName, failedJob.jobId)
          return job.status
        },
        { timeout: 15000 }
      )
